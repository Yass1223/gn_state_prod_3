...
# Line classes already reported as having no symmetric counterpart, so the warning
# below is emitted once per class name rather than once per annotated frame.
_UNMIRRORABLE_REPORTED = set()
...
def mirror_labels(lines_dict):
    ...
    Classes absent from ``SoccerPitch.symetric_classes`` are skipped with a warning
    instead of raising. A subscript here made one unmirrorable class abort the whole
    metrics run; the cost of skipping is that line's contribution to the mirrored
    variant, which is the smaller error by far.
    """
    mirrored_dict = dict()
    for line_class, value in lines_dict.items():
        mirror = SoccerPitch.symetric_classes.get(line_class)
        if mirror is None:
            if line_class not in _UNMIRRORABLE_REPORTED:
                _UNMIRRORABLE_REPORTED.add(line_class)
                log.warning(
                    f"[sn_calibration] line class {line_class!r} has no entry in "
                    f"SoccerPitch.symetric_classes; it is dropped from the mirrored "
                    f"variant. Its contribution to the mirrored score is lost."
                )
            continue
        mirrored_dict[mirror] = value
