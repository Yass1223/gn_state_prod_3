# SoccerNet Game State Reconstruction — pipeline reference (repo_prod_v3)

Reference date: 2026-09-06. Derived from direct inspection of the repository
...
descriptors; outliers by MUTUAL REACHABILITY of the 2-means team cores (appearance_outliers_plain_v2); geometry-first
...
condition); `role_team` computes from SINGLE rows only and writes the decided
labels on EVERY row of the trajectory, with min_n 10 (goalkeeper/main only),
gk_rel 0.85, assistants without a detection threshold (one per touchline, up
to two with opposite mean-y; the two-different-assistants rule decided here,
not in the merger) and the jersey `vote_pool` vote tie broken by the best
single crop's maxconf. Not yet exercised on Kaggle.
Intended to
...
is `https://github.com/Yass1223/gn_state_prod_3` (main). Canonical copy:
...
```
bbox_detector -> track -> crop_filter -> calibration -> pitch_gate
             -> team_embed -> jersey_number_detect -> traj_refine -> role_team
             -> tracklet_agg -> audit
(tracklet_split DROPPED 2026-09-20: the merger works directly on the tracker's
 post-gate tracklets; multi-crop detections are ignored until traj_refine stage 3)
```
...
* torchreid is the VlSomers/bpbreid fork (git dependency); it provides `osnet_ain_x1_0`
  (tracker/traj_refine embedder) and `osnet_x1_0` (team model backbone).
...
| `calibration` | `sn_gamestate.calibration.broadtrack_api.BroadTrackCalibration` | BroadTrack binary at `pretrained_models/broadtrack/`; camera prior (0, 55, −12); `min_score 0.3` rejects lost frames and reuses the last accepted camera (`use_prev_parameters: true`, `max_carry_frames 0`); per-sequence JSON cache `broadtrack_calib/` (`use_cached_json: true`); the binary is nondeterministic across runs (~9 GS-HOTA points of draw spread measured on SNGS-116), so on a cache miss it runs `calib_attempts` (3) times and the attempt whose accepted frames have the highest mean line-IoU score is kept (`<seq>.selection.json` sidecar records every attempt + winner; `calib_attempts: 1` = the old single run; see `docs/CALIBRATION_FIX.md`); writes human-bbox masks from own detections; `staging_dir` for read-only datasets; emits camera `parameters` and `bbox_pitch` |
...
| `role_team` | `sn_gamestate.team.RoleTeamAssignment` | Per-TRAJECTORY roles and sides, AFTER `traj_refine` (the last labelling stage; `team/rules.py` and its notebook-equivalence test are untouched). The stage RECOMPUTES appearance itself from the finished trajectories' CLEAN crops (the fragment embeddings are gone: `traj_refine` drops them; `team_cluster`/`team_cluster_nearest` are inert snapshots, not read). **Since 2026-09-06 every statistic and condition is computed from SINGLE (`crop_single`) rows only, and the decided role/team labels are written on EVERY row of the trajectory** — multi rows inherit their trajectory's labels (for metrics and visualization; count in sidecar `n_multi_rows_labelled`). Per sequence: geometry stats per trajectory on the stride-5 grid over single rows (mean/std x,y; q75 of \|x\|; y-range; sampled count); one osnet_team descriptor per trajectory (≤ crops_per_track 16 sampled single crops, L2-normalised median; same checkpoint coordinates as `team_embed`, digest in the sidecar); APPEARANCE OUTLIERS over the descriptors by MUTUAL REACHABILITY of the team clusters (appearance_outliers_plain_v2 — ONE rule replacing the previous MAD-threshold + DBSCAN channels; sole parameter link_k 4): 2-means supplies the assignment and centroid distances d; per team the CORE = members with d ≤ median + MAD (floored) — certain team members, inliers by definition; each tracklet's own RADIUS R_i = median + MAD (floored) of its link_k nearest-neighbour distances; tracklets are MUTUALLY COMPATIBLE iff δᵢⱼ ≤ min(Rᵢ, Rⱼ); OUTLIER ⟺ no chain of mutually compatible tracklets links it to either core (`outlier_group`); skipped, nothing flagged, when n_desc ≤ link_k + 1. Role order: **ASSISTANTS FIRST** — no resolved jersey number, NO sampled-count minimum (min_n does not apply), touchline geometry (\|mean y\| ≥ tau_a 0.9 · clip max, y-std ≤ tau_a_sy 3.0), one winner per y-sign → up to TWO assistants with opposite mean-y; the TWO-DIFFERENT-ASSISTANTS rule is decided here on the final trajectories (deliberately NOT a merge condition): both winners are assigned, their expected appearance closeness (same kit) is recorded as `assistant_pair_appearance_d` (cosine distance of the two descriptors), and they are never collapsed into one. GOALKEEPERS: no resolved number, sampled count ≥ min_n 10, depth rule \|mean x\| ≥ gk_rel 0.85 · the maximum non-assistant \|mean x\| (reference in sidecar `gk_depth_ref`); one per half. MAIN referee (one per sequence): no resolved number, sampled count ≥ min_n, y-range inside the band of the trajectory means — rule (2.14): max(y_ref) ≤ band 0.9 · max_i(mean_y_i) AND min(y_ref) ≥ band · min_i(mean_y_i) — the candidate CLOSEST IN APPEARANCE (cosine) to the assistants; with no assistant, the candidate farthest from both k-means centroids. **min_n gates goalkeepers and the main referee only.** Everyone else a player (a leftover outlier stays a player, `player_outlier`). Sides: 2-means REFIT on the non-outlier player descriptors (all player descriptors when < 2 clean); every player with a descriptor takes its nearest refit centroid; clusters named left/right by the cue chain over player mean-x (`side_rule: keeper` — positional keeper cue, then quantile (20 %/80 % average), then mean; `vote` = majority); a player with no descriptor takes its mean-x half (flagged fallback). tau_a/tau_a_sy/link_k/band/min_n/gk_rel UNTUNED on this pipeline's trajectories. Sidecar `audit/role_team/` (embedder provenance incl. sha256; per_trajectory role/why/team/cluster/in_core/radius/outlier/d; sequence_level cues/band/main_referee(+rule)/gk_selection/gk_depth_ref/assistant_pair_appearance_d/appearance_outliers (rule, n_desc, link_k, skipped, per-team size-mu-s-core, n_linked)/outlier_group/n_multi_rows_labelled/counts) |
| `jersey_number_detect` | `sn_gamestate.jersey.jn_gsr_api.JNGsrTrackletRecognizer` | subprocess workers in the 3.10 venv; since batch 7 eligibility = at least one single crop, NO role filter (roles do not exist yet; the legibility filter is what discards referee crops); `single_crops_only: true`; legibility > 0.72 → DBNet++ ROI → PARSeq + SATRN → `vote_pool` (the only rule; since 2026-09-06 a VOTE TIE between labels is broken by the best single-crop decode confidence — pooled mx — then total strength, then label); stride 5; fp16; GPU sharding auto via nvidia-smi (2 workers on Kaggle 2×T4); content-hash cache `jn_cache/`. Since 2026-09-03 (blob **schema 2**): two ADDITIVE columns for `traj_refine` — `jersey_number_candidates` (every pooled label of the two recognisers as `[label, mx, conf_sum, votes]`, ranked by the maxconf score exp(mx)·conf_sum; stats, not scores, so merged tracklets recombine exactly: mx=max, conf_sum/votes add) and `jersey_number_maxconf` (assigned number's score). The schema is folded into the cache key (old caches miss and recompute once) and checked on every shard and cached blob; the assigned number stays `vote_pool`, byte-identical. The batch-7 eligibility change alters the manifest content, so EVERY sequence's cache key changes and the entire cache recomputes once (referee fragments now enter the workers) |
| `traj_refine` | `sn_gamestate.refine.traj_refine_api.TrajRefine` | The pipeline's ONE merge plus stage-3 duplicate-frame resolution, between jersey and role_team. **Rebuilt 2026-09-06 as a THREE-PHASE merger over a partition by label knowledge; since 2026-09-20 it works DIRECTLY on the tracker's post-gate tracklets (the splitter stage is dropped)** — S1: cluster AND number known; S2: cluster known, number unknown; a tracklet with NO cluster id (no single crop at all, or all sampled crops failed to embed) belongs to no partition and NEVER merges. Phase S1 (label-driven, NO distance threshold): all same-cluster same-number pairs in descending JOINT pooled maxconf (exp(max mx)·Σconf_sum); clean frame sets disjoint → merge; overlap → conflict — the lower-maxconf side walks to its best candidate not previously lost (banned set; cascades; "-1"/exhaustion → unnumbered, moves to S2). Phase S2: agglomerative (median centroid over clean detections, recomputed over the full membership after every merge) under same cluster + clean-frame disjointness, distance ≤ tau. FINAL phase: all S1+S2 survivors pooled, same conditions + no contradicting numbers (two DIFFERENT known numbers never merge; numbered + unnumbered may), distance ≤ tau. **There is NO re-enter condition** (dropped with its `use_reenter`/`edge_margin` keys and boxes/image-width inputs): tau is the only guard on time-disjoint same-cluster pairs. Ghost rule: multi-crop detections enter no condition and no centroid; their one exception is stage 3 — 3a keeps one detection per (frame, trajectory) (clean wins; else nearest the clean-first centroid) and holds the rest; 3b places held detections in ascending distance into the nearest trajectory with that frame free, DYNAMIC centroids (recomputed after every assignment), rest unassigned (`track_id` NaN, the ONLY way any refinement stage drops a row). Same OSNet-AIN pin as track (audit-enforced; tau 0.60 untuned for this stage). A merged cluster unifies `team_cluster`; the resolved jersey number/confidence/maxconf are written to ALL member rows; rows adopted in 3b take the target cluster's labels. Snapshots written unconditionally: `track_id_prerefine`, `jersey_number_detection_prerefine`, `jersey_number_confidence_prerefine`, `team_cluster_prerefine`; `enabled: false` = snapshots + sidecar only (the A/B switch). The ENABLED path DROPS `team_embedding` (cleared on every row; `outputs.team_embedding_dropped`): after the merge, trajectories carry the jersey evidence only, and `role_team` re-embeds from clean crops. Output invariant: one detection per (image_id, track_id) over ALL detections; tracked rows out = in − unassigned. Sidecar `audit/traj_refine/` (`partition`, `phase_s1` incl. conflict_log, `phase_s2`, `phase_final`, `merges_s1/s2/final`, merge log with phase, `stage3`, `no_centroid`, `out_of_scope`, `rows_unassigned`, per-cluster `team_cluster`); algorithm in `refine/traj_refine.py` (pure numpy, 21 unit tests in `tests/test_traj_refine.py`) |
...
| `audit` | `sn_gamestate.audit.RunAudit` | read-only last stage; per-sequence, per-component PASS/WARN/FAIL to `audit/<seq>.json`; cross-checks every sidecar against the composed config (including track vs traj_refine checkpoint-pin equality); since batch 13 the verdicts are INFORMATION for inspection — nothing blocks on them: `scripts/verify_run_integrity.py` prints them and its exit code is driven only by execution failures (logged subprocess failures, missing/unreadable artifacts, sequence-coverage shortfalls). Batch-7 basis map (the snapshot chain of the new order): `track_id_pregate` = TRACKER ids (basis for the track, crop_filter and calibration checks); `track_id_prerefine` = the gate's output (basis for the gate's id comparison and the team_embed and jersey checks); final `track_id` = trajectories (basis for the role_team, traj_refine-output and tracklet_agg checks). `_check_team_embed` validates the clustering: cluster_method/outlier_k ran == configured, sizes sum to clustered (under `kmeans2_nearest`: every embedded fragment clustered, no unclustered-with-descriptor), per-fragment cluster constancy and {0,1} values recomputed from the columns — on the pre-refine basis the cluster column is read from the `team_cluster_prerefine` SNAPSHOT when present, since traj_refine legitimately rewrites the live column (run-9 false-FAIL fix, batch 8) — clustered count == sidecar; since batch 10 the per-fragment COVERAGE is audited via `team_cluster_nearest` (coverage == the sidecar's `cluster.embedded`, checkable when ≥ 2 fragments embedded; `team_embedding` itself is legitimately cleared by traj_refine, so the column no longer carries evidence), missing descriptors == `fragments_no_single`. `_check_role_team` audits the FINAL trajectories directly (role_team is the last labelling stage): a valid role on EVERY tracked row (multi rows included, since labels are written trajectory-wide), role/side constant per trajectory over ALL its rows, `n_multi_rows_labelled` cross-checked against the columns, referees sideless, ≤ 1 main_2.14 referee and ≤ 2 assistants (algorithm invariants — the code guarantees them by construction, so a violation means malfunction or tampering; the both-teams and keeper-cap SCENE expectations are REMOVED in batch 13, scene composition is recorded in `observed` as information only), `outlier_group` == the per-trajectory outlier flags and `n_outlier` == its length (batch 10), appearance_outliers (mutual-reachability) bookkeeping sound — no outlier flags when the rule was skipped, flags ≤ descriptors, linked + flagged == descriptors, and no team-core member flagged (cores are inliers by definition), half-fallback count consistent with the per-trajectory reasons (half fallback ⇒ WARN; the nearest-centroid fallback check is retired with the fallback), params want ⊆ ran, sidecar covers every trajectory. `_check_jersey` eligibility = single-crop presence (no role filter). The `traj_refine` check compares settings (enabled + tau) and the checkpoint pin, verifies merges == S1 + S2 + final and every S2/final merge distance ≤ tau (S1 merges are label-driven, no threshold), and does row accounting (rows losing an id == sidecar `rows_unassigned`, no row may gain one, tracked_after == tracked_before − unassigned) with the pin-equality key `ain_sha256_track` and number/`team_cluster` constancy per final track |
...
1. `traj_refine`'s tau was tuned on embeddings from a notebook export whose
   producing checkpoint cannot be verified from this repository; if it was not
   the pinned OSNet-AIN, it does not transfer and needs re-tuning on `valid`.
...
7. UPDATED 2026-09-06: `traj_refine` `tau` (0.60) is now the pipeline's ONLY merge
   ...
   same embedder pin and distance convention; it is NOT tuned for this stage. With the
   re-enter condition dropped (§10), tau is the only guard on time-disjoint same-cluster
   pairs — tune on `valid` if the Kaggle test shows over/under-merging.
...
item 4), and tune on `valid` — `traj_refine` tau (§7 item 7) and the
role_team geometry thresholds (§3), carried from ground-truth-tracklet tuning and
untuned on this pipeline's trajectories.

---

## 10. Redesign of 2026-09-06

A coordinated redesign of the refinement chain, applied and unit-tested but NOT yet
exercised on Kaggle **[unverified]** — §3 rows for `tracklet_split`, `team_embed`,
...
3. **Merger rebuilt** (`traj_refine`): THREE phases over a label partition — S1
   (cluster+number known; same-cluster same-number pairs by descending joint pooled
   maxconf; disjoint → merge with NO distance threshold; overlap → maxconf conflict
   cascade, an unnumbered loser joins S2), S2 (cluster known/number unknown;
   agglomerative within tau), FINAL (all survivors pooled; + no contradicting
   numbers; within tau). A fragment with no cluster id never merges.
4. **Re-enter DROPPED** entirely: no exit/entry-side condition in any phase;
   `use_reenter`/`edge_margin` config keys, the S1 set-aside/`rejected_s1` machinery,
   the nearest-side helpers and the boxes/image-width inputs all removed (algorithm,
   api, audit, tests, configs). tau is now the only guard on time-disjoint
   same-cluster pairs (§7 item 7).
5. **Jersey evidence trajectory-wide**: the merger writes the resolved
   number/confidence/maxconf to ALL member rows. The `vote_pool` VOTE TIE is broken
   by the best single-crop decode confidence (pooled mx), then total strength, then
   label.
6. **`role_team` single-only + trajectory-wide**: every statistic and condition from
   SINGLE rows only; the decided labels written on EVERY row (multi rows inherit;
   `n_multi_rows_labelled`). ASSISTANTS first — no resolved number, NO sampled-count
   minimum, touchline geometry (tau_a 0.9, tau_a_sy 3.0), one per y-sign → up to two
   with opposite mean-y; the two-different-assistants rule decided HERE (not a merge
   condition), pair appearance distance recorded (`assistant_pair_appearance_d`).
   GOALKEEPERS: gk_rel 0.85 · max non-assistant \|mean x\| + min_n 10. MAIN referee:
   no number + min_n + band (2.14) + appearance closest to the assistants. min_n
   gates goalkeepers and the main referee ONLY.
...
8. Unit suites after the redesign: 16 (`test_tracklet_split.py`) + 21
   ...
   `plugins/jn_gsr/fuse_jn.py` + worker, `audit/run_audit_api.py`, the module
   configs, `configs/soccernet.yaml`, tests, and `docs/kaggle_one_sequence_test.ipynb`
   (repinned to `gn_state_prod_3`).
